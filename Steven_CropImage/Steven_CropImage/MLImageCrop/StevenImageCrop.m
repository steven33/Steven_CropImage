...
@interface StevenImageCrop ()<UIScrollViewDelegate>
...
@implementation StevenImageCrop
...
- (void)viewDidLoad
{
    ...
    UIButton *sender;
    [self.view addSubview:sender =[self acquireCustomButtonWithTitle:@"确认" andAction:@selector(changeImage111)]];
    
    sender.frame = CGRectMake(0, 64, 100, 40);
...
- (void)changeImage111{
    self.isXuanzuan = !self.isXuanzuan;
    UIImage *markImage = [StevenImageCrop image:self.imageView.image rotation:UIImageOrientationLeft ];
    self.image = markImage;
...
- (void)onConfirm:(id)sender
{
    ...
    //    if (self.isXuanzuan == YES) {
    //        wRatio = self.imageView.image.size.width/(self.imageView.frame.size.width/self.scrollView.zoomScale);
    //        hRatio = self.imageView.image.size.height/(self.imageView.frame.size.height/self.scrollView.zoomScale);
    //    }
    ...
    if (self.isXuanzuan == YES) {
        ...
        cropRect = CGRectMake(startPoint.x*wRatio, startPoint.y*hRatio, (endPoint.x-startPoint.x)*wRatio, (endPoint.x-startPoint.x)*wRatio);
